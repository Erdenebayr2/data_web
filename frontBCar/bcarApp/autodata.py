...
async def fetch_page(page_number, session):
    mlink = f'https://www.unegui.mn/avto-mashin/-avtomashin-zarna/?page={page_number}'
...
async def process_page(page_url, session, data):
    async with session.get(page_url) as response:
        ...
        new_keys = ['Advantage_payment_amount', 'Engine_capacity', 'Gearbox', 'Hurd', 'Type', 'Color', 'Year_of_manufacture', 'Year_of_entry',
                        'Engine', 'Interior_color', 'Leasing', 'Address', 'Drive', 'Mileage', 'Conditions', 'Gate']
        
        new_dict = {new_key: dict[old_key] for old_key, new_key in zip(dict.keys(), new_keys)}

        data[zar_num[0]] = {'Manufacturer': manufacturer ,'Mark': mark, 'date': ogno, 'price': price,**new_dict,'Description':desc}
...
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        with open('datas/data.json', 'w') as file:
